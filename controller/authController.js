...
const createSendToken = (user, statusCode, res) => {
  ...
  const expiresDate = new Date(Date.now() + cookieExpiresIn * 24 * 60 * 60 * 1000);
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  // 1. Lấy user dựa trên email POSTed
  ...
  try {
    await sendEmail({
      ...
      name: user.name || 'Người dùng', // Sử dụng tên người dùng nếu có
      resetLink: resetURL // Truyền liên kết reset
    });
